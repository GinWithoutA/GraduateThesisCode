...
"""
def main_solver(ego_pla, ego_pla_state, desire_velocity, pre_pla_state = -1, ego_pre_distance_to_conflict_point = -1):
    ...
    for step in range(predictive_horizon):
        for index, vehicle in enumerate(ego_pla_state):
            ...
            if index == 0:
                if pre_pla_state != -1:
                    # 当前是LVP Member
                    p_cost_function += (pre_pla_state[step][0] - position[index, step] + desire_space + pre_pla_state[step][2]) * (pre_pla_state[step][0] - position[index, step] + desire_space + pre_pla_state[step][2])
