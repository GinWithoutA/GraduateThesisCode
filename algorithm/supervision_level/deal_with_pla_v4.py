'''
    算法版本4：
        不管是 LVP Leader 还是 LVP Member 传递的值都应该是行驶的距离，若没有冲突点，直接传递行驶的距离加上出现的位置，若
        有冲突点，则传递的是路径起始点到冲突点的距离减去已经行驶的距离再减去出现的位置
    （可行）
'''
...
'''
    ...
    Return
        None
'''
def update_current_spacing_error(traci, ego_pla, pre_pla, step, all_vehicle_id):
    if platoon[pre_pla][0][-1] in all_vehicle_id:
    ...
    if platoon[ego_pla][0][0] in all_vehicle_id:
        ...
        if (ego_pla_leader_edge == edge_beg and get_origin_distance(traci, pla_leader, traci.lane.getShape(traci.vehicle.getLaneID(pla_leader))[1]) > 236.4) or (ego_pla_ender_edge == edge_end and get_origin_distance(traci, pla_ender, traci.lane.getShape(traci.vehicle.getLaneID(pla_ender))[0]) > 236.4):
            ...
            if platoon[ego_pla][1] != -1:
                ...
                if platoon_flag[ego_pla][2] != enum_pf.pass_conflict_already_flag.PASSING:
                    if platoon_flag[ego_pla][3] == enum_pf.ego_pre_same_lane_flag.DIFFERENT:
                        ...
                        if ego_distance < 0:
                            ...
                            update_current_spacing_error(traci, ego_pla, platoon[ego_pla][1], step, all_vehicle_id)
